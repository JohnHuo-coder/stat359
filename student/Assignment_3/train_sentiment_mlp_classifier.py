...
def get_sentence_ft_embedding(sentence):
    fasttext_emb = fasttext_model.vectors
    fasttext_word2idx = fasttext_model.key_to_index
    words = tokenize_text(sentence)
    result_embeddings = []
    for word in words:
        if word in fasttext_word2idx:
            idx = fasttext_word2idx[word]
            result_embeddings.append(fasttext_emb[idx])
    if not result_embeddings:
        return np.zeros(fasttext_emb.shape[1])
...
def run_test():
    ...
    model = MLP(300, NUM_CLASSES).to(device)
    checkpoint_path = "outputs/mlp_model.pth"
    ...
    plot_confusion_matrix(
        ...
        save_path="outputs/mlp_test_confusion_matrix_normalized.png",
        normalize=True
    )
    ...
    plot_confusion_matrix(
        ...
        all_preds,
        class_names=["Neg", "Neu", "Pos"],
        save_path="outputs/mlp_test_confusion_matrix.png",
        normalize=False
    )
    ...
    plot_f1_loss_acc_bar(
        ...
        macro_f1_score,
        test_acc,
        save_path = "outputs/mlp_test_metric.png"
    )
...
def run_training():
    ...
    torch.save({
        ...
        'val_F1': val_f1_score,
        'train_F1': train_f1_score
    }, "outputs/mlp_last_model.pth")
        
    plot_metrics(all_train_losses, all_val_losses, 'Loss', 'outputs/mlp_loss_curve.png')
    plot_metrics(all_train_f1, all_val_f1, 'Macro F1', 'outputs/mlp_f1_curve.png')
    plot_metrics(all_train_acc, all_val_acc, 'Accuracy', 'outputs/mlp_accuracy_curve.png')
    # Clear memory
